...
const WWScaper = (() => {
    ...
    const _HEADER_LABEL_MAP = {
        'id':                      'jobId',    // Full-Cycle All Jobs board
        'job id':                  'jobId',
        'job #':                   'jobId',
        'posting id':              'jobId',    // My Applications board
        'posting #':               'jobId',
        'job posting id':          'jobId',
        'job title':               'title',    // Full-Cycle All Jobs board
        'level':                   'level',    // Full-Cycle All Jobs board (Junior/Intermediate/Senior)
        'apps':                    'apps',     // Full-Cycle All Jobs board (number of applicants)
        'term':                    'term',
        'term level':              'term',
    ...
    function scrapeColumnHeaders() {
        ...
        // Some columns (e.g. Select All checkbox, Job ID) live in <th> or non-table__value
        // cells in data rows rather than <td class="table__value">. Derive the skip count
        // dynamically so the colMap indices always align with the actual cells[] array.
        const firstRow = document.querySelector('tr.table__row--body');
        const dataCellCount = firstRow ? firstRow.querySelectorAll('td.table__value').length : 0;
        const skipCount = dataCellCount > 0 ? headerCells.length - dataCellCount : 1;

        const colMap = {};
        headerCells.forEach((th, i) => {
            if (i < skipCount) return;
            const tdIndex = i - skipCount;
    ...
    function scrapeListingRow(tr, colMap) {
        const titleAnchor =
            tr.querySelector('th .overflow--ellipsis a')
            ?? tr.querySelector('th a')
            ?? tr.querySelector('td.table__value .overflow--ellipsis a')
            ?? tr.querySelector('td.table__value a');
        const cells = tr.querySelectorAll('td.table__value');

        const cell = (i) => {
            const td = cells[i];
            if (!td) return '';
            return (td.querySelector('.overflow--ellipsis')?.textContent ?? td.textContent).trim();
        };
        ...
        // Job ID: use colMap when available (Direct board — ID is in a td.table__value).
        // Full-Cycle All Jobs board puts the ID in a <th>, so search <th> elements instead.
        // My Applications board: ID is in the first <td> but colMap lacks a jobId entry
        // (header label doesn't match our map), so scan first few cells as last resort.
        const jobId = (useDynamic && colMap.jobId != null)
            ? cell(colMap.jobId)
            : (() => {
                for (const el of tr.querySelectorAll('th')) {
                    const n = (el.querySelector('.overflow--ellipsis')?.textContent ?? el.textContent)
                        .replace(/\D/g, '').trim();
                    if (n.length >= 4) return n;
                }
                // Scan first few <td> cells for a purely-numeric job ID (My Applications).
        ...
        return {
            ...
            openings:     parseInt(field('openings', 4), 10) || null,
            city:         field('city',          5),
            level:        field('level', 6).split(/\n/).map(s => s.trim()).filter(Boolean).join(', '),
            apps:         (n => isNaN(n) ? null : n)(parseInt(field('apps', 7), 10)),
            appDeadline:  field('appDeadline',   8),
        };
    }
    ...
    function parseListingRow(apiRow) {
        ...
        return {
            ...
            apps:        typeof kv.numApplicants === 'number' ? kv.numApplicants : null,
    ...
    function scrapeJobDetailFromHtml(html, jobId, title, employer, division) {
        try {
            ...
            // Scope to the job-detail panel the same way scrapeJobDetail() does for the live modal.
            // Without this, _scrapeFields runs on the full 36KB page HTML and picks up every
            // tag__key-value-list block on the page (navigation, listing rows, etc.), producing
            // a bloated jobData that fails the backend's schema validation.
            const modal = doc.querySelector('div.modal.is--visible[role="dialog"]')
                       ?? doc.querySelector('div[role="dialog"]')
                       ?? doc.querySelector('.dashboard-header__posting-title')?.closest('div')
                       ?? doc.body;
            const panel  = modal.querySelector('div[role="tabpanel"][aria-labelledby="overview"]') ?? modal;
            const fields = _scrapeFields(panel);
    ...
    function _normalizeDetailFields(fields) {
        ...
        // "Job - Province/State" → "jobProvincestate" after _toFieldKey removes the slash
        if (!fields.province) {
            const k = keys.find(k => k === 'jobProvincestate' || (k.startsWith('job') && k.toLowerCase().includes('province')));
            if (k && fields[k]) fields.province = fields[k];
        }
