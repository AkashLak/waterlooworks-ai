...
const WWScaper = (() => {
    ...
    function scrapeListingRow(tr) {
        // WaterlooWorks DOM — verified April 2026, confirm still accurate on May 9th
        // Column 1 is a <th>, all remaining columns are <td class="table__value">
        const titleAnchor = tr.querySelector('th.overflow--hidden a.overflow--ellipsis');
     ...
     * Confirmed DOM structure — April 2026:
     *   div.tag__key-value-list
    ...
    function _scrapeFields(container) {
        // WaterlooWorks DOM — verified April 2026, confirm still accurate on May 9th
        const result = {};
        const blocks = container.querySelectorAll('div.tag__key-value-list');
     ...
     * Collapses horizontal whitespace (spaces/tabs) without destroying newlines,
     * then collapses multiple consecutive newlines to one.
     * Applied before any text reaches the AI to improve output quality.
     */
    function _cleanText(text) {
        return text
            .replace(/[^\S\n]+/g, ' ')  // collapse spaces/tabs → single space, leave newlines
    ...
    function extractJobDescription(detail) {
        if (!detail) return '';
        // Clean each section individually so the \n\n paragraph separator is preserved
    ...
    function _trimBoilerplate(text) {
        // Drops company/HR boilerplate that starts at "About the Business Unit"
