...
const WWScaper = (() => {
    ...
    function parseListingRow(apiRow) {
        ...
        return {
            jobId:          String(kv.jobId ?? ''),
            title:          kv.jobTitle     ?? '',
            employer:       kv.organization ?? '',
            organization:   kv.organization ?? '',
            term:           kv.term         ?? '',
            appStatus:      kv.applicationStatus ?? '',
            jobStatus:      kv.jobStatus    ?? '',
            division:       kv.division     ?? '',
            location:       kv.loc          ?? '',
            city:           kv.city         ?? null,
            openings:       typeof kv.numPos === 'number' ? kv.numPos : null,
            appDeadline:    kv.applicationDeadline ?? null,
            boardUrl:       kv.postingModule ?? null, // e.g. "/myAccount/co-op/direct/jobs.htm"
        };
    }
     ...
     *
     * WaterlooWorks uses labels like "Job - City", "Job - Country", and
     * "Job Location (If Exact Address Unknown or Multiple Locations)". _toFieldKey
     * converts these to "jobCity", "jobCountry", and a 50-char key respectively.
     * This function promotes them to the clean names handlers.js expects.
     */
    function _normalizeDetailFields(fields) {
        // "Job - City" → city (fallback when listing row has no city)
        if (!fields.city && fields.jobCity)
            fields.city = fields.jobCity;

        // "Job Location (If Exact Address Unknown or Multiple Locations)" → city fallback
        // Applies when there's no single "Job - City" (e.g. IBM: Toronto/ Ottawa/ Montreal)
        if (!fields.city) {
            const multiLoc = fields.jobLocationIfExactAddressUnknownOrMultipleLocations ?? '';
            if (multiLoc) fields.city = multiLoc;
        }

        // "Job - Country" → country
        if (!fields.country && fields.jobCountry)
            fields.country = fields.jobCountry;
