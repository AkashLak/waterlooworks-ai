...
const WWStorage = (() => {
    const KEYS = {
        ...
        DEVICE_ID:            'ww_device_id',        // random UUID generated once, salts resume hash
        OPENAI_KEY:           'ww_openai_key',        // optional BYOK key — stored locally, never logged
        AUTH_SESSION:         'ww_auth_session',
    };
    ...
    async function getOrCreateDeviceId() {
        let id = await _get(KEYS.DEVICE_ID);
        if (!id) {
            id = crypto.randomUUID();
            await _set(KEYS.DEVICE_ID, id);
        }
        return id;
    }
    ...
    async function getApiKey() {
        return (await _get(KEYS.OPENAI_KEY)) ?? null;
    }

    /** @param {string} key */
    async function saveApiKey(key) {
        await _set(KEYS.OPENAI_KEY, key);
    }

    async function clearApiKey() {
        await _set(KEYS.OPENAI_KEY, null);
    }

    async function getAuthSession() { return (await _get(KEYS.AUTH_SESSION)) ?? null; }
    async function saveAuthSession(session) { await _set(KEYS.AUTH_SESSION, session); }
    async function clearAuthSession() { await _set(KEYS.AUTH_SESSION, null); }
