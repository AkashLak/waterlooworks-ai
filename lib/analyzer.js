...
const WWAnalyzer = (() => {
    ...
    async function analyzeBatch(onProgress, onResult, onComplete) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                // Skip jobs with no meaningful description (closed jobs, empty fields)
                if (!jobDescription || jobDescription.length < 80) {
                    onResult?.({ row, jobId: row.jobId, error: 'No job description available' });
                    continue;
                }
    ...
    function _clickJobTitle(el) {
        if (!el) return;
        // Click the <a> directly so Vue's click handler fires and opens the modal.
        // This causes a CSP warning in chrome://extensions (WaterlooWorks's own page
        // CSP blocks the javascript:void(0) href navigation) but that is developer-only
        // and harmless — the modal opens correctly regardless.
