...
const WWAnalyzer = (() => {
    ...
    async function analyzeBatch(onProgress, onResult, onComplete) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                // Dispatch a synthetic click so Vue's handler fires without the browser
                // attempting to navigate the href="javascript:void(0)", which WaterlooWorks's
                // own page CSP would block and log as an error.
                row.titleEl?.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true }));
