...
// which is the only context that calls the backend.
...
const WWAnalyzer = (() => {
    ...
    function _send(payload) {
        return new Promise((resolve, reject) => {
            chrome.runtime.sendMessage(payload, (response) => {
                ...
                if (!response?.success) {
                    if (response?.errorType === 'quota_exceeded') _refreshCreditStatus?.();
                ...
                if (['getFitScore', 'getDreamFit', 'searchJobs', 'askQuestion'].includes(payload.action)) {
    ...
    function bulkDescriptions(jobs) {
        return _send({ action: 'bulkDescriptions', jobs });
    }
     ...
     */
    function getDreamFit(jobId, dreamCriteria) {
        return _send({ action: 'getDreamFit', jobId, dreamCriteria });
    }

    function createReport(feature, input, output, note) {
        return _send({ action: 'createReport', feature, input, output, note });
    }
     ...
     */
    function syncActiveJobs(rows, term, isFiltered) {
        return _send({ action: 'syncActiveJobs', rows, term, isFiltered });
    }
    ...
    function getQuotaStatus() {
        return _send({ action: 'getQuotaStatus' });
    }

    function getAuthState() {
        return _send({ action: 'getAuthState' });
    }

    /** Bulk-syncs locally-cached fit scores to Supabase. Fire-and-forget from init. */
    function syncFitScores() {
        return _send({ action: 'syncFitScores' });
    }
