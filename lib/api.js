 ...
 * Requests use the signed-in user's short-lived Supabase access token.
 */
const WWApi = (() => {
    const TIMEOUT_MS = 30_000;

    async function _headers() {
        const token = await WWAuth.getAccessToken();
        return {
    ...
    async function _request(method, path, body) {
        ...
        try {
            ...
            clearTimeout(timer);
            if (err.type === 'auth_required' || err.type === 'auth_config' || err.type === 'auth') throw err;
        ...
        if (!response.ok) {
            ...
            if (body.error === 'auth_required' || body.error === 'waterloo_email_required' || body.error === 'email_not_confirmed') {
                throw _err(body.message ?? 'Please sign in again.', 'auth_required');
            }
            ...
            if (body.error === 'invalid_api_key') {
                throw _err(body.message ?? 'Invalid API key — check your key in Settings.', 'invalid_api_key');
            }
    ...
    function _statusMessage(status) {
        if (status === 401) return 'Please sign in again to use WatAssistant';
        if (status === 413) return 'Job details upload is too large. Please try again.';
     ...
     */
    async function getFitScore(jobId, resumeText, resumeHash, apiKey) {
        return _request('POST', '/api/fit-score', { jobId, resumeText, resumeHash, ...(apiKey ? { apiKey } : {}) });
    }
     ...
     */
    async function getDreamFit(jobId, resumeText, dreamCriteria, resumeHash, apiKey) {
     ...
     */
    async function searchJobs(resumeText, criteria, resumeHash, apiKey) {
        return _request('POST', '/api/jobs/search', { resumeText, resumeHash, ...criteria, ...(apiKey ? { apiKey } : {}) });
    }
     ...
     */
    async function askQuestion(jobId, question, resumeText, apiKey, resumeHash) {
        return _request('POST', `/api/jobs/${encodeURIComponent(jobId)}/ask`, { question, resumeText, resumeHash, ...(apiKey ? { apiKey } : {}) });
    }
